...
class Explainer:
    ...
    def make_pred_real(self, adj, start):
        # house graph
        if self.args.dataset == "sat1" or self.args.dataset == "sat2":
...
class ExplainModule(nn.Module):
    ...
    def log_adj_grad(self, node_idx, pred_label, epoch, label=None):
        ...
        # only for graph mode since many node neighborhoods for sat tasks are relatively large for
        # visualization
        if self.graph_mode:
            G = io_utils.denoise_graph(
